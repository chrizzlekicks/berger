"""TmuxSandbox: an isolated, throwaway tmux server for smoke-testing berger.
...
Safety invariants (see plan doc for the empirical validation behind these):
  ...
  * Every berger invocation happens with a `tmux` shim script prepended to
    PATH. The shim execs REAL_TMUX (the resolved absolute path, not a bare
    `tmux`) with `-L <this sandbox's socket>` spliced in, so berger -- which
...
BERGER_BIN = os.path.realpath(
    os.path.join(os.path.dirname(__file__), "..", "..", "target", "debug", "berger")
)
...
def _real_tmux(*args, **kwargs):
    ...
    harness-owned setup/teardown, never on berger's behalf."""
...
class TmuxSandbox(contextlib.AbstractContextManager):
    ...
    def __init__(self, *, dead=False):
        ...
        # every tmux call berger makes fails and it takes the documented
        ...
        self.socket = f"berger-test-{os.getpid()}-{n}"
        self._tmpdir = tempfile.mkdtemp(prefix="berger-smoke-")
    ...
    def __enter__(self):
        ...
        # started a throwaway bootstrap session first, berger's own
    ...
    # -- berger invocations (always through the shim) ------------------------
    ...
    def _env_prefix(self, extra_env):
        ...
        # berger's config-dir resolution inside the sandbox.
        return f"XDG_CONFIG_HOME= {assignments}"

    def event(self, payload_json, *, session, window, agent=None, timeout=5.0):
        """Run `berger event` inside a run-shell targeted at session:window, so
        ...
        temp file and rename it into place only after `berger event` exits --
        ...
        `berger event` itself exited zero.
        ...
        displayed, not what `#W` resolves to, and berger's own window
        ...
        extra = {"BERGER_AGENT": agent} if agent is not None else {}
        ...
        # be expanded by the inner `sh -c` before berger ever sees stdin.
        payload_path = os.path.join(self._tmpdir, f"payload-{next(_socket_counter)}.json")
        with open(payload_path, "w") as f:
            f.write(payload_json)
        inner = (
            f"cat {_sh_quote(payload_path)} | "
            f"{prefix} {_sh_quote(BERGER_BIN)} event >{_sh_quote(tmp_path)} 2>&1; "
            f"printf \"\\n__EXIT__:%s\\n\" \"$?\" >>{_sh_quote(tmp_path)}; "
        ...
        # agent) -- callers passing `agent=` rely on BERGER_AGENT for identity
        ...
        if not wait_for(_done, timeout=timeout):
            raise TimeoutError(
                f"berger event did not finish within {timeout}s "
        ...
        if exit_code != 0:
            raise RuntimeError(f"berger event exited {exit_code}: {output}")
        return output

    def run_berger(self, *args, stdin=None, timeout=10):
        """Run a berger subcommand as a plain (shimmed) subprocess -- used for
    ...
    def state_path(self, session, agent):
        ...
        return os.path.join(self.xdg_cache, "berger", session_component, f"{agent_component}.state")
...
def _escape_path_component(component):
    ...
    %XX, byte-for-byte (berger operates on raw bytes, not code points)."""
    encoded = []
